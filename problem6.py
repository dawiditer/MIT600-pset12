# -*- coding: utf-8 -*-
"""
Created on Fri Jun 09 20:19:11 2017
...
def problem6():
    ...
    # TODO
    start = time.time()
#    hist1 = numpy.array(trialForNTimeSteps(0))
#    hist2 = numpy.array(trialForNTimeSteps(75))
    hist3 = numpy.array(trialForNTimeSteps(150))
#    hist4 = numpy.array(trialForNTimeSteps(300))
    end = time.time()
    print (int(end - start))/60,"minutes"

#    bins = numpy.array(range(0,1100,100))
    pyplot.hist(hist3)
    pyplot.yticks(range(0, 35, 5))
    pyplot.title("Delayed Cocktail Treatment by 150 Time steps")
...
def problem4_6():
    ...
    #let the virus particles do their thing for 150 time steps
    for i in xrange(1,300):
        #induce drug after 150 time steps
        if i >= 150: 
            PatientZero.addPrescription('guttagonol')
            PatientZero.addPrescription('grimpex')
#        #wait for another 75 timesteps then induce grimpex
#        if i >= 225: PatientZero.addPrescription('grimpex')
        population_after_time.append(PatientZero.update())
    ...
    pyplot.plot(resistant_pop_after_time,label = "Resistant Population")
    pyplot.scatter(150,population_after_time[150],label = "guttagonol Induced",color='k')
    pyplot.scatter(150,population_after_time[150],label = "grimpex Induced",color='g')
    pyplot.title("Virus Population vs. Time")
    pyplot.ylabel("Total Virus Population")
    pyplot.xlabel("Time")
    pyplot.legend()
    pyplot.show()        
    print PatientZero.getPrescriptions()
